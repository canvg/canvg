 ...
 */
 (function ( global, factory ) {
	...
	//       dpi: int => scales images to match dpi resolution (default is 72 dpi, set to 300 for print resolution)
		...
		svg.loadXmlDoc = function(ctx, dom) {
			...
			var draw = function() {
				...
				if (ctx.canvas.parentNode) {
					svg.ViewPort.SetCurrent(ctx.canvas.parentNode.clientWidth*svg.dpi, ctx.canvas.parentNode.clientHeight*svg.dpi);
				} else {
				...
				if (svg.opts['ignoreDimensions'] != true) {
					...
					if (e.style('width').hasValue()) {
						ctx.canvas.width = e.style('width').toPixels('x')*svg.dpi;
						if (ctx.canvas.style) {ctx.canvas.style.width = ctx.canvas.width*svg.dpi + 'px';}
					}
					if (e.style('height').hasValue()) {
						ctx.canvas.height = e.style('height').toPixels('y')*svg.dpi;
						if (ctx.canvas.style) {ctx.canvas.style.height = ctx.canvas.height*svg.dpi + 'px';}
					}
				}
